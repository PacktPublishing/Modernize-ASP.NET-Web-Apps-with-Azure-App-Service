...
namespace MvcMusicStore.Controllers
{
    ...
    {
        ApiHelper orderApiHelper, shoppingCartApiHelper;
        const string PromoCode = "FREE";


        public CheckoutController(IConfiguration _config)
        {
            orderApiHelper = new ApiHelper(_config.GetValue<string>("Services:MvcOrderService"));
            shoppingCartApiHelper = new ApiHelper(_config.GetValue<string>("Services:MvcShoppingCartService"));
        }
            ...
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        int orderId = await orderApiHelper.PostAsync<OrderCreation, int>("/api/Checkout/AddressAndPayment", creation);
        ...
        public async Task<ActionResult> Complete(int id)
        {
            try
            {
                ...
                bool isValid = await orderApiHelper.GetAsync<bool>(string.Format("/api/Checkout/Complete?id={0}&username={1}", id, username));
                if (isValid)
                {
                    return View(id);
                }
